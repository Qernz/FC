...
// para novos tipos
float f(float x) {return pow(cos(x),2);}
...
class Vec3d // para vectores num espaco a 3 dimensoes
      ...
      public: // para usar fora da classe
             ...
             friend float operator*(const Vec3d& r1, const Vec3d& r2 );
             friend Vec3d operator-(const Vec3d& r);
             ...
             void print(const Vec3d r)
             {
                 cout << r.r[0] << " " << r.r[1] << " " << r.r[2] << " " << endl;
             }
...
Vec3d operator+( const Vec3d& r1, const Vec3d& r2 ) {
      Vec3d ret;
      for( int i = 0; i < 3; ++i )
           ret.r[i] = r1.r[i] + r2.r[i]; // como o operador é amigo da classe pode aceder à parte privada de Vec3d
      return ret;
      }
...
float operator*(const Vec3d& r1, const Vec3d& r2)
    {
        float ret = 0;
        for (int i = 0; i < 3; i ++)
        {
...
int main()
{
    ...
    f = x*y;
    z = x;
    cout << Vec3d(1,3,2) * Vec3d(1,1,1) << endl;
    cout << x << endl;
    cout << z << endl;
    z = x-y;
    cout << z << endl;*/
